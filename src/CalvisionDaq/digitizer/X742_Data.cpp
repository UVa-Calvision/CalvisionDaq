        ...
        for(j=0; j<8; j++){
            if (i==1) {
                ...
                if ((i==N_Samples-1)&&((raw_data(j,N_Samples-2) - raw_data(j,N_Samples-1) )>30)) {
                    offset++;										 							        
                } else if ((raw_data(j,i-1) - raw_data(j,i) )>30) {
...
template <typename T>
SampleArray<FloatingType> unwrap_drs(const T* src, size_t starting_index) {
    SampleArray<FloatingType> dst;
    const size_t n = N_Samples - starting_index;
    copy_buffer<FloatingType>(dst.data(), src + starting_index, n);
    copy_buffer<FloatingType>(dst.data() + starting_index, src + n, starting_index);
    return dst;
}
...
void x742GroupData::ApplyDataCorrection(CAEN_DGTZ_DRS4Correction_t* CTable, int CorrectionLevelMask)
{
    ...
    for (i=0;i<N_Channels;i++) {
        for (j=0;j<N_Samples;j++) {
            if (cellCorrection) {
                // SampleArray<FloatingType> offset = unwrap_drs(CTable->cell[i], st_ind);
                // cblas_saxpy(N_Samples, -1.0, offset.data(), 1, channel_data[i].data(), 1);
                channel_data[i][j] -= CTable->cell[i][((st_ind+j) % 1024)]; 
            }
            if (nsampleCorrection) {
                // cblas_saxpy(N_Samples, -1.0, CTable->nsample[i], 1, channel_data[i].data(), 1);
