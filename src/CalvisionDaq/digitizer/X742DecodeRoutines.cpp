...
static int V1742UnpackEventGroup(uint32_t /* group */, uint32_t *datain, CAEN_DGTZ_X742_GROUP_t *dataout) {
    ...
    int i, rpnt = 0, wpnt = 0, size1, size2,trg = 0,k;
    uint16_t st_ind=0; 
...
/*
 * HRH: Modified to assume the given event pointer is already allocated so we don't
 * need to do extra mallocs. This also lets TTree read directly from the event instead
 * of needing extra copies.
 */
int32_t X742_DecodeEvent(char *evtPtr, CAEN_DGTZ_X742_EVENT_t *Event) {
