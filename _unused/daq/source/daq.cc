...
int main(int argc, char **argv) {
  ...
  if(!no_verbose)
    std::cout << "Readout Buffer Size (bytes): " << size << std::endl; 
  //----- open fstream
  std::ofstream fout;
...
int setup() {
  ...
  if(0) {
    ...
    CAEN_DGTZ_SetPostTriggerSize(fHandle,50); // percentage of record length
  }
  ...
  if(1) {
    ...
    reg |= 0xff; // channels 00001111 (first four channels)
    ...
    reg |= (0xf000) | 0xa5a;  //setting threshold for all channels (0xf) // a24
    fLastErrorCode = CAEN_DGTZ_WriteRegister(fHandle, 0x1080, reg);

    CAEN_DGTZ_ReadRegister(fHandle, 0x1180, &reg);
    reg |= (0xf000) | 0xa5a;  //setting threshold for all channels (0xf)
    fLastErrorCode = CAEN_DGTZ_WriteRegister(fHandle, 0x1180, reg);

    // Post Trigger (time delay)
    CAEN_DGTZ_SetPostTriggerSize(fHandle,0); // percentage of record length
  }  
    
  // shutting down SW and EX trigger
  //CAEN_DGTZ_ReadRegister(fHandle, 0x810C, &reg);
  //reg = (reg<<2) >> 2; // clear last two bits 
  //CAEN_DGTZ_WriteRegister(fHandle, 0x810C, reg);



  // Front Pannel Control
  //fLastErrorCode = CAEN_DGTZ_ReadRegister(fHandle, 0x811C, &reg);
  //uint32_t reg = (3<<18)|(1<<16); /// makes GPO send 'busy' signal
  //fLastErrorCode = CAEN_DGTZ_WriteRegister(fHandle, 0x811C, reg);
  
  // MUST BE 1
  //CAEN_DGTZ_ReadRegister(fHandle, 0x8000, &reg);
  //reg |= 0b1000;
  //CAEN_DGTZ_WriteRegister(fHandle, 0x8000, reg);
...
void print_info() {
  ...
  fLastErrorCode = CAEN_DGTZ_ReadRegister(fHandle, 0x8000, &reg);
