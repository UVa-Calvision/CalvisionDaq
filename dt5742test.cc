...
int main(int argc, char **argv) {
  ...
  //ret = CAEN_DGTZ_SetPostTriggerSize(handle, 48);
  ret = CAEN_DGTZ_SetPostTriggerSize(handle, 10);
  if (ret != CAEN_DGTZ_Success) {
    std::cerr << "Failed to change post trigger size" << std::endl;
  }

  uint32_t posttrig = 0;
  ret = CAEN_DGTZ_GetPostTriggerSize(handle, &posttrig);
  if (ret != CAEN_DGTZ_Success) {
    std::cerr << "Failed to get post trigger size" << std::endl;
  }
  if (ret == CAEN_DGTZ_Success) 
    std::cout << "Current post-trigger size:" << posttrig << "%." << std::endl;
